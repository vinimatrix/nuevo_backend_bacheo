...
export class AuthService2 {
  ...
  async validateLogin(
    ...
    if (
      ...
      const registerDto = {
        ...
        firstName: loginDto.password,
        lastName: '',
      };

      await this.register(registerDto);

      const reg_user = await this.usersService.findOne({
  ...
  async register(dto: AuthRegisterLoginPhoneDto): Promise<void> {
    const hash = crypto
      .createHash('sha256')
